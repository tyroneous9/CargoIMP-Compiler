...
IATA Cargo-IMP is a format that's still in daily production use across the air cargo industry. It's not easy to parse by manually writing parsers: a single message type has several incompatible revisions that all need to be parsed (`FFM/4`, `FFM/5`, `FFM/8` all show up), field boundaries are positional, and structure is conditional, e.x. a "loose"/bulk FFM shipment just omits its ULD line instead of having an empty line (which would be easier to parse).

To address this format's complexity, each message type is treated as a formal language: an ABNF grammar defines what is structurally valid for a revision. This grammar is human readable and can be used to directly translate the rules of the messages. The actual parsers are generated by an external library based on the parsers. These parsers are what actually read messages and output parsed fields.

## Parsers explained

Each grammar (`cpp/data/grammars/*.abnf`) compiles through [aParse](https://www.parse2.com/) into its own C++ parser binary, one executable per message type. They're built as separate binaries because aParse doesn't namespace its generated code, so combining multiple grammars into a single binary would risk symbol collisions between them. 
...
Compiling a grammar produces two layers. `cpp/data/generated_parsers/` (build artifact so gitignored) is aParse's direct output: C++ code that parses a message into a tree mirroring the grammar's rules. It's regenerated completely on every grammar change. `cpp/src/*_json_extractor.cpp` is separate file that reads from that generated tree and pulls fields out into a fixed, usable JSON shape used by the actual data pipeline.

This two-layer structure means small changes in grammar could cause significant change in the generated tree. The json extractor must be updated correctly to match the changes in grammar in order to walk through the generated tree, producing the correct parsed output in the form of JSON keys.

### Standalone CLI

Each parser binary is a standalone CLI tool which can take a message via `-file <path>` or `-string <text>` and prints JSON to stdout:
...
Given a raw message on disk, it deterministically produces JSON or a diagnosed error with nothing else running, which makes it easy to embed in a different pipeline or use as a standalone CIMP decoding tool on its own. This is the same binary the pipeline shells out to (`spawnSync`, `server/scripts/pipeline/parser-runner.js`).
...
> `docs/CIMP-34thEdition2014.pdf`/`.txt` are the IATA/A4A Cargo-IMP manual which is a licensed publication, gitignored for distribution reasons. Get your own copy from IATA if you need the format reference; `docs/cimp-input-differences.md` covers the project's own notes on format variance without reproducing the manual.
