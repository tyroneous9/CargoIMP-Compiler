...
-- 9) Notification status by MAWB (notification-only subjects from emails_raw).
...
WHERE raw_json->'recognizedNotification'->>'eventType' IN ('rcf', 'arrival_notice', 'delivery_complete', 'ready_for_pick_up', 'dlv', 'nfd');
...
WITH notification_events AS (
	...
	FROM emails_raw er
	WHERE er.raw_json->'recognizedNotification'->>'eventType' IN ('rcf', 'arrival_notice', 'delivery_complete', 'ready_for_pick_up', 'dlv', 'nfd')
		AND er.raw_json->'recognizedNotification'->>'mawb' IS NOT NULL
), notification_rollup AS (
	SELECT
		ne.mawb_number,
		BOOL_OR(ne.event_type = 'rcf') AS has_rcf,
		BOOL_OR(ne.event_type = 'arrival_notice') AS has_arrival_notice,
...
SELECT
	...
	COALESCE(nr.has_arrival_notice, FALSE) AS has_arrival_notice
FROM mawb_universe mu
LEFT JOIN notification_rollup nr ON nr.mawb_number = mu.mawb_number
ORDER BY mu.mawb_number;

-- report_mawb final shape includes MAWB-level notification signals.
CREATE OR REPLACE VIEW report_mawb AS
SELECT
	f.id AS fwb_master_id,
	f.mawb_number,
...
-- 10) Pipeline runtime tracking for Option A single-service scheduler/worker.
