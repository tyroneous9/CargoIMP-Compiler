...
Rule_OciContLine::Rule_OciContLine(
  const string& spelling, 
  const vector<const Rule*>& rules) : Rule(spelling, rules)
{
}

Rule_OciContLine::Rule_OciContLine(const Rule_OciContLine& rule) : Rule(rule)
{
}

Rule_OciContLine& Rule_OciContLine::operator=(const Rule_OciContLine& rule)
{
  Rule::operator=(rule);
  return *this;
}

const Rule_OciContLine* Rule_OciContLine::clone() const
{
  return new Rule_OciContLine(this->spelling, this->rules);
}

void* Rule_OciContLine::accept(Visitor& visitor) const
{
  return visitor.visit(this);
}

const Rule_OciContLine* Rule_OciContLine::parse(ParserContext& context)
{
  context.push("OciContLine");
  ...
  if (parsed)
  {
    rule = new Rule_OciContLine(context.text.substr(a0.start, a0.end - a0.start), a0.rules);
  }
  else
  {
    context.index = s0;
  ...
  context.pop("OciContLine", parsed);

  return (Rule_OciContLine*)rule;
}
