...
const Rule_HouseBillLine* Rule_HouseBillLine::parse(ParserContext& context)
{
  ...
  parsed = false;
  {
    ...
    if (parsed)
    {
      ...
      {
        const Rule* rule = Terminal_StringValue::parse(context, "HBS");
        if ((f1 = rule != NULL))
        {
    ...
    if (parsed)
    {
      ...
      for (int i1 = 0; i1 < 1 && f1; i1++)
      {
        const Rule* rule = Rule_HouseOriginAndDestination::parse(context);
        if ((f1 = rule != NULL))
        {
          a1.add(*rule, context.index);
          c1++;
    ...
    if (parsed)
    {
      ...
      for (int i1 = 0; i1 < 1 && f1; i1++)
      {
        const Rule* rule = Rule_HouseWeightUnit::parse(context);
        if ((f1 = rule != NULL))
        {
          a1.add(*rule, context.index);
          c1++;
    ...
    if (parsed)
    {
      ...
      for (int i1 = 0; i1 < 1 && f1; i1++)
      {
        const Rule* rule = Rule_HouseWeight::parse(context);
        if ((f1 = rule != NULL))
        {
          a1.add(*rule, context.index);
          c1++;
