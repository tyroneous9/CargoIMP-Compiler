...
Rule_AgentTagLine::Rule_AgentTagLine(
  const string& spelling, 
  const vector<const Rule*>& rules) : Rule(spelling, rules)
{
}

Rule_AgentTagLine::Rule_AgentTagLine(const Rule_AgentTagLine& rule) : Rule(rule)
{
}

Rule_AgentTagLine& Rule_AgentTagLine::operator=(const Rule_AgentTagLine& rule)
{
  Rule::operator=(rule);
  return *this;
}

const Rule_AgentTagLine* Rule_AgentTagLine::clone() const
{
  return new Rule_AgentTagLine(this->spelling, this->rules);
}

void* Rule_AgentTagLine::accept(Visitor& visitor) const
{
  return visitor.visit(this);
}

const Rule_AgentTagLine* Rule_AgentTagLine::parse(ParserContext& context)
{
  context.push("AgentTagLine");
  ...
  parsed = false;
  {
    ...
    if (parsed)
    {
      ...
      {
        const Rule* rule = Terminal_StringValue::parse(context, "AGT");
        if ((f1 = rule != NULL))
        {
  ...
  if (parsed)
  {
    rule = new Rule_AgentTagLine(context.text.substr(a0.start, a0.end - a0.start), a0.rules);
  }
  else
  {
    context.index = s0;
  ...
  context.pop("AgentTagLine", parsed);

  return (Rule_AgentTagLine*)rule;
}
