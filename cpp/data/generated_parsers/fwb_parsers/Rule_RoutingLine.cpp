...
Rule_RoutingLine::Rule_RoutingLine(
  const string& spelling, 
  const vector<const Rule*>& rules) : Rule(spelling, rules)
{
}

Rule_RoutingLine::Rule_RoutingLine(const Rule_RoutingLine& rule) : Rule(rule)
{
}

Rule_RoutingLine& Rule_RoutingLine::operator=(const Rule_RoutingLine& rule)
{
  Rule::operator=(rule);
  return *this;
}

const Rule_RoutingLine* Rule_RoutingLine::clone() const
{
  return new Rule_RoutingLine(this->spelling, this->rules);
}

void* Rule_RoutingLine::accept(Visitor& visitor) const
{
  return visitor.visit(this);
}

const Rule_RoutingLine* Rule_RoutingLine::parse(ParserContext& context)
{
  context.push("RoutingLine");
  ...
  parsed = false;
  {
    ...
    if (parsed)
    {
      ...
      {
        const Rule* rule = Terminal_StringValue::parse(context, "RTG");
        if ((f1 = rule != NULL))
        {
  ...
  if (parsed)
  {
    rule = new Rule_RoutingLine(context.text.substr(a0.start, a0.end - a0.start), a0.rules);
  }
  else
  {
    context.index = s0;
  ...
  context.pop("RoutingLine", parsed);

  return (Rule_RoutingLine*)rule;
}
