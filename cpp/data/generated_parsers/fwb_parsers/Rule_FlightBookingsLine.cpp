...
Rule_FlightBookingsLine::Rule_FlightBookingsLine(
  const string& spelling, 
  const vector<const Rule*>& rules) : Rule(spelling, rules)
{
}

Rule_FlightBookingsLine::Rule_FlightBookingsLine(const Rule_FlightBookingsLine& rule) : Rule(rule)
{
}

Rule_FlightBookingsLine& Rule_FlightBookingsLine::operator=(const Rule_FlightBookingsLine& rule)
{
  Rule::operator=(rule);
  return *this;
}

const Rule_FlightBookingsLine* Rule_FlightBookingsLine::clone() const
{
  return new Rule_FlightBookingsLine(this->spelling, this->rules);
}

void* Rule_FlightBookingsLine::accept(Visitor& visitor) const
{
  return visitor.visit(this);
}

const Rule_FlightBookingsLine* Rule_FlightBookingsLine::parse(ParserContext& context)
{
  context.push("FlightBookingsLine");
  ...
  parsed = false;
  {
    ...
    if (parsed)
    {
      ...
      {
        const Rule* rule = Terminal_StringValue::parse(context, "FLT");
        if ((f1 = rule != NULL))
        {
    ...
    if (parsed)
    {
      ...
      while (f1)
      {
        const Rule* rule = Rule_ContinuationLine::parse(context);
        if ((f1 = rule != NULL))
        {
          a1.add(*rule, context.index);
          c1++;
  ...
  if (parsed)
  {
    rule = new Rule_FlightBookingsLine(context.text.substr(a0.start, a0.end - a0.start), a0.rules);
  }
  else
  {
    context.index = s0;
  ...
  context.pop("FlightBookingsLine", parsed);

  return (Rule_FlightBookingsLine*)rule;
}
