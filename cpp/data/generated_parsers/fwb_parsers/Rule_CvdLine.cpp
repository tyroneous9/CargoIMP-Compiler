...
Rule_CvdLine::Rule_CvdLine(
  const string& spelling, 
  const vector<const Rule*>& rules) : Rule(spelling, rules)
{
}

Rule_CvdLine::Rule_CvdLine(const Rule_CvdLine& rule) : Rule(rule)
{
}

Rule_CvdLine& Rule_CvdLine::operator=(const Rule_CvdLine& rule)
{
  Rule::operator=(rule);
  return *this;
}

const Rule_CvdLine* Rule_CvdLine::clone() const
{
  return new Rule_CvdLine(this->spelling, this->rules);
}

void* Rule_CvdLine::accept(Visitor& visitor) const
{
  return visitor.visit(this);
}

const Rule_CvdLine* Rule_CvdLine::parse(ParserContext& context)
{
  context.push("CvdLine");
  ...
  parsed = false;
  {
    ...
    if (parsed)
    {
      ...
      {
        const Rule* rule = Terminal_StringValue::parse(context, "CVD");
        if ((f1 = rule != NULL))
        {
      ...
      for (int i1 = 0; i1 < 1 && f1; i1++)
      {
        ...
        parsed = false;
        {
          ...
          if (parsed)
          {
            ...
            {
              const Rule* rule = Terminal_StringValue::parse(context, "NVD");
              if ((f2 = rule != NULL))
              {
      ...
      for (int i1 = 0; i1 < 1 && f1; i1++)
      {
        ...
        parsed = false;
        {
          ...
          if (parsed)
          {
            ...
            for (int i2 = 0; i2 < 1 && f2; i2++)
            {
              const Rule* rule = Terminal_StringValue::parse(context, "NCV");
              if ((f2 = rule != NULL))
              {
                a2.add(*rule, context.index);
                c2++;
      ...
      for (int i1 = 0; i1 < 1 && f1; i1++)
      {
        ...
        parsed = false;
        {
          ...
          if (parsed)
          {
            ...
            for (int i2 = 0; i2 < 1 && f2; i2++)
            {
              const Rule* rule = Terminal_StringValue::parse(context, "XXX");
              if ((f2 = rule != NULL))
              {
                a2.add(*rule, context.index);
                c2++;
  ...
  if (parsed)
  {
    rule = new Rule_CvdLine(context.text.substr(a0.start, a0.end - a0.start), a0.rules);
  }
  else
  {
    context.index = s0;
  ...
  context.pop("CvdLine", parsed);

  return (Rule_CvdLine*)rule;
}
