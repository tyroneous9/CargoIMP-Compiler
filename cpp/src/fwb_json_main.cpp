...
int main(int argc, char* argv[])
{
  ...
  if (!ok)
  {
    parser_logging::logError("parser_fwb_json", std::string("argument error: ") + error);
    std::cerr << "error: " << error << std::endl;
    std::cerr << "usage: parser_fwb_json [-rule rulename] [-trace] <-file file | -string string>" << std::endl;
  }
  else
  {
    try
    {
      if (arguments.find("File") != arguments.end())
      {
        ...
        if (!file.is_open())
        {
          parser_logging::logError("parser_fwb_json", std::string("io error: unable to open ") + arguments["File"]);
    ...
    catch (ParserException& e)
    {
      ...
      parser_logging::logError("parser_fwb_json", std::string("parser error: ") + detail);
      std::cerr << "parser error:\n" << detail << std::endl;
    }
    catch (std::exception& e)
    {
      parser_logging::logError("parser_fwb_json", std::string("runtime error: ") + e.what());
      std::cerr << "error: " << e.what() << std::endl;
    }
    catch (...)
    {
      parser_logging::logError("parser_fwb_json", "unknown error");
